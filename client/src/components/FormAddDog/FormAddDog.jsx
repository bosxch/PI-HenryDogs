...
import { getTemperaments, postDog } from "../../store/actions/index.js";
...
const validate = (form) => {
    ...
    if (form.name) {
        for (let i=0 ; i < form.name.length ; i++) {
            if (regex.test(form.name[i])) errors.name = 'It should not contian numbers'
        }
    }
    // --height validate-- //
    ...
    for (let i=0 ; i < form.min_height.length ; i++){
        if (regex.test(form.min_height[i] === false)) {
            errors.height = 'The height cannot had letters'
        }
    }
    for (let i=0 ; i < form.max_height.length ; i++){
        if (regex.test(form.max_height[i] === false)) {
            errors.height = 'The height cannot had letters'
        }
    }
    if (form.min_height > form.max_height) {
       errors.height = 'The min height cannot be higher than the max height'
    }
    // --weight validate-- //
    if(!form.min_weight || !form.max_weight) {
        errors.weight = "Weight is required"
    }
    for (let i=0 ; i < form.min_weight.length ; i++){
        if (regex.test(form.min_weight[i] === false)) {
            errors.weight = 'The height cannot had letters'
        }
    }
    for (let i=0 ; i < form.max_weight.length ; i++){
        if (regex.test(form.max_weight[i] === false)) {
            errors.weight = 'The height cannot had letters'
        }
    }
    if (form.min_weight > form.max_weight) {
...
// --life_span validate-- //
    ...
    if (!form.life_span.includes('-')) {
        errors.life_span = 'Lifespan has to have a ' - ' between the two ages'
    }
    if (form.life_span.split('-')[0] > form.life_span.split('-')[1]) {
        errors.life_span = 'Ages must be from youngest to oldest. Example 10-15'
    }
    if (form.life_span > 25) {
        errors.life_span = 'Such high values are not allowed'
    }
